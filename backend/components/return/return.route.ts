...
router.get('/',
  ...
  query('status').optional().custom((value: string) => Object.values(ReturnStatus).includes(value as ReturnStatus)),
...
router.put('/:returnId',
  ...
  param('returnId').isNumeric().toInt(),
  body('status').optional().custom((value: string) => Object.values(ReturnStatus).includes(value as ReturnStatus)),
